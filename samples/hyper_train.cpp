// Modified from "https://github.com/darglein/NeAT"
   ...
   public:
        ...
        : params(params), experiment_dir(experiment_dir)
    {
        ...
        for (auto scene_name : params->train_params.scene_name)
        {
            ...
            TrainScene ts;
            ts.neural_geometry = std::make_shared<GeometryOCT>(scene->num_channels, scene->D, tree, params);

            ts.scene = scene;
            ts.tree  = tree;

            if(params->octree_params.tree_optimizer_params.optimize_tree_roi_at_ini )
            {
                auto node_min = tree->node_position_min;
                auto node_max = tree->node_position_max;
                auto node_mid = node_min + node_max;

                std::cout << "before active node num " << tree->NumActiveNodes() << std::endl;
                ...
                std::cout << " region is " << scene->dataset_params.roi_min << " " << scene->dataset_params.roi_max << std::endl;
                for(int i = 0; i < tree->NumActiveNodes();++i)
                {
                    ...
                    if(!in_roi(node_min_ptr, node_max_ptr, node_mid_ptr, params->octree_params.tree_optimizer_params.tree_ini_roi_min, params->octree_params.tree_optimizer_params.tree_ini_roi_max))
                    {
                    ...
                    if (params->train_params.output_volume_size > 0)
                    {
                        ...
                        if (epoch_id == params->train_params.num_epochs)
                        {
                            ...
                            auto [volume_density, volume_node_id, volume_valid] = neural_geometry->UniformSampledVolume(
                                {out_size, out_size, out_size}, scene->num_channels,scene->dataset_params.roi_min, scene->dataset_params.roi_max, true );
                            ...
                            auto volume_slice = volume_density_crop.slice(2, params->train_params.volume_slice_view, params->train_params.volume_slice_view+1);
                            ...
                            im1_new.save(ep_dir + "/" + scene->scene_name +  "imageslice.png");
   ...
   private:
    ...
    {
        {
            ...
            for(int i = 0; i < tree->NumActiveNodes();++i)
            {
                ...
                float* node_max_ptr = node_max.data_ptr<float>()+ node_id * node_min.stride(0);
                float* node_mid_ptr = node_mid.data_ptr<float>()+ node_id * node_min.stride(0);
                if(in_roi(node_min_ptr, node_max_ptr, node_mid_ptr, tree_roi_min, tree_roi_max))
                {
    ...
    void TrainStep(int epoch_id, bool train_indices, std::string name, bool only_image_params)
    {
        ...
        for (SampleData sample_data : (*data_loader))
        {
            ...
            SampleList all_samples = tree->CreateSamplesForRays(rays, params->octree_params.max_samples_per_node,
            ...
            auto per_ray_loss_mse =
                ((predicted_image - sample_data.pixels.target) * params->train_params.loss_scale_before).square();
            auto per_ray_loss_l1 =
                ((predicted_image - sample_data.pixels.target) * params->train_params.loss_scale_before).abs();

            auto per_ray_loss = (per_ray_loss_mse * params->train_params.lose_mse_scale +
                                 per_ray_loss_l1 * params->train_params.lose_l1_scale) *
                              ...
                              std::string checkpoint_name, bool test, bool calculate_std)
    {
        ...
        if (!test && params->octree_params.optimize_structure && calculate_std)
        {
            ...
            if(params->octree_params.tree_optimizer_params.use_tree_roi)
            {
                ...
                // float * node_max_ptr = node_max.data_ptr<float>();
                // float * node_mid_ptr = node_mid.data_ptr<float>();
                // long int *  active_nodes_id = tree->active_node_ids.to(torch::kCPU).data_ptr<long>();
                auto active_nodes_id = tree->active_node_ids.to(torch::kCPU);
                // cell_loss_combined = cell_loss_combined.to(torch::kCPU);
        ...
        // torch::Tensor predicted_image_all = torch::empty({0, projection_images[0].size(1), projection_images[0].size(2)});
        // torch::Tensor target_image_all = torch::empty({0, projection_images[0].size(1), projection_images[0].size(2)});
        ...
        // SaveHDRImageTensor(predicted_image_all.unsqueeze(0), ep_dir + "/" + scene->scene_name + "_reproj_inv.hdr");
        // SaveHDRImageTensor(target_image_all.unsqueeze(0), ep_dir + "/" + scene->scene_name + "_original_inv.hdr");
...
int main(int argc, const char* argv[])
{
    ...
    if (0)
    {
        ...
        //                                                 torch.tensor([[[1,0,0],[0,1,0.]]]), (1,1,5,5),
