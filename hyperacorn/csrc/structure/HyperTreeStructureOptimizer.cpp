    ...
    : tree(tree), params(params)
{
    ...
    if(!solver)
    {
        std::cout << "solver unavilable " << std::endl;
    }

    // solver->EnableOutput(); // new add enable output debug
...
bool HyperTreeStructureOptimizer::OptimizeTree()
{
    ...
    for (PerNodeData& nd : data)
    {
        ...
        // int split = round(nd.I_split->solution_value());
        // int grp   = round(nd.I_grp->solution_value());
        // int none  = round(nd.I_none->solution_value());
...
void HyperTreeStructureOptimizer::CreateVariables()
{
    ...
    for (int i = 0; i < tree->NumActiveNodes(); ++i)
    {
        ...
        // node_data.I_split = solver->MakeIntVar(0,1,"");
        // node_data.I_none  = solver->MakeIntVar(0,1,"");
        // node_data.I_grp   = solver->MakeIntVar(0,1,"");
...
void HyperTreeStructureOptimizer::CreateConstraints()
{
    ...
    vec3 tree_roi_min = params.tree_edge_roi_min;
    vec3 tree_roi_max = params.tree_edge_roi_max;
    ...
    for (int i = 0; i < tree->NumActiveNodes(); ++i)
    {
        ...
        // std::cout << tree->node_position_min.data_ptr<float>()[node_id * 3 ] 
        //           << tree->node_position_min.data_ptr<float>()[node_id * 3 + 1] 
        //           << tree->node_position_min.data_ptr<float>()[node_id * 3 + 2] << std::endl;
        PerNodeData& node_data = data[node_id];
...
void HyperTreeStructureOptimizer::CreateObjective()
{
    ...
    for (int i = 0; i < tree->NumActiveNodes(); ++i)
    {
        ...
        if (children_ids[0] >= 0 && in_roi(node_min_ptr, node_max_ptr, node_mid_ptr, tree_roi_min, tree_roi_max))
        {
