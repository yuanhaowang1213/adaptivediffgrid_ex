...
torch::Tensor HyperTreeBaseImpl::CreateSamplesRandomly(torch::Tensor node_id,const int num_of_samples_per_edge)
{
    ...
    torch::Tensor rand_value = torch::rand({N, num_of_samples_per_edge, num_of_samples_per_edge, num_of_samples_per_edge,D()},torch::TensorOptions(device).dtype(torch::kFloat));
    #if 0
    {
        Printf("Should not run\n");
        auto result_position = torch::zeros({N, num_of_samples_per_edge, num_of_samples_per_edge, num_of_samples_per_edge,D()},torch::TensorOptions(device).dtype(torch::kFloat));
...
{
    int num_samples = size.array().prod();
    torch::Tensor global_coordinate  = torch::empty({num_samples, D()}, torch::TensorOptions(torch::kFloat32)).to(torch::kCUDA);
...
static __global__ void NodeNeighborSamples(DeviceHyperTree<D> tree, Eigen::Vector<int, D> size, float epsilon,
                                           ...
                                           StaticDeviceTensor<long, 1> out_node_id,
                                            vec3 roi_min, vec3 roi_max, bool use_roi)
{
...
SampleList HyperTreeBaseImpl::NodeNeighborSamples(Eigen::Vector<int, -1> size, double epsilon , vec3 roi_min, vec3 roi_max, bool use_roi)
{
    ...
    switch (D())
    {
        case 3:
            ::NodeNeighborSamples<3><<<NumActiveNodes(), 128>>>(this, size, epsilon, out_num_samples.data_ptr<int>(),
                                                                ...
                                                                list.ray_index, list.local_index_in_ray, list.node_id, roi_min, roi_max, use_roi);
            break;
        default:
            CHECK(false);
    }
